...
<!-- *Recursive*, *Convolutional* and *Meta-Learning* layers -->
...
<!-- $\tilde{\vec{h}}_v^{(l)}(t) =\sum\limits_{u \in N(v;t)} \vec{\sigma} \left(\frac{\vec{q}^{(l-1)}_{v(t)}^t K_{N(v;t)}^{(l-1)}(t)}{\sqrt{d}}\right) \vec{V}_u(t)^{(l-1)}(t)$ -->

<!-- \text{softmax} \left( \frac{Q^{(l-1)}K^{(l-1)}^T}{\sqrt{d_K}} \right)V -->
